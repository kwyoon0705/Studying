...
public class Ex002 {
	@Test
	public void test() {
		assertArrayEquals(solution(new String[] { "3 - 4 = -3", "5 + 6 = 11" }), new String[] { "X", "O" });
		assertArrayEquals(solution(new String[] { "19 - 6 = 13", "5 + 66 = 71", "5 - 15 = 63", "3 - 1 = 2" }),
				new String[] { "O", "O", "X", "O" });
	}
	...
	public static String[] solution(String[] quiz) {
		...
		for (int i = 0; i < quiz.length; i++) {
			...
			switch (ar[1]) {
			case "+":
				int exp1 = Integer.parseInt(ar[0]) + Integer.parseInt(ar[2]);
				answer[i] = exp1 == Integer.parseInt(ar[4]) ? "O" : "X";
				break;
			case "-":
				int exp2 = Integer.parseInt(ar[0]) - Integer.parseInt(ar[2]);
				answer[i] = exp2 == Integer.parseInt(ar[4]) ? "O" : "X";
				break;
			}
		}
		return answer;
